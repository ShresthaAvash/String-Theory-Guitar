...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (user != null) {
            ...
            String productToAddId = (String) session.getAttribute("productToAddAfterLoginId");
            String productToAddQuantity = (String) session.getAttribute("productToAddAfterLoginQuantity");

            if (productToAddId != null && productToAddQuantity != null) {
                session.removeAttribute("productToAddAfterLoginId");
                session.removeAttribute("productToAddAfterLoginQuantity");

                String addToCartUrl = request.getContextPath() + "/cart?" +
                                      "action=addToCart" +
                                      "&productId=" + URLEncoder.encode(productToAddId, StandardCharsets.UTF_8.toString()) +
                                      "&quantity=" + URLEncoder.encode(productToAddQuantity, StandardCharsets.UTF_8.toString());
                response.sendRedirect(addToCartUrl);
                return;
            }

            String redirectParam = request.getParameter("redirect");
            if (redirectParam != null && !redirectParam.isEmpty() &&
                !redirectParam.equals(request.getContextPath() + "/login") &&
                !redirectParam.startsWith(request.getContextPath() + "/login?")) {
